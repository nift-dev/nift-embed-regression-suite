...
python3 -m json.tool "$EMPTY/.nift/tracked.json" >/dev/null 2>&1 || fail 'saving zero tracked files produces invalid tracked.json'
...
python3 -m json.tool "$QUOTED/.nift/tracked.json" >/dev/null 2>&1 || fail 'track title containing double quotes corrupts tracked.json (JSON escaping bug candidate)'
...
if (cd "$INFOQUOTE" && "$NIFT_BIN" build-names quote-title >/dev/null 2>&1); then
  python3 -m json.tool "$INFOQUOTE/.nift/public/quote-title.info.json" >/dev/null 2>&1 || fail 'quoted title corrupts generated page info JSON'
else
...
if (cd "$INFONAME" && "$NIFT_BIN" track 'q"name' >/dev/null 2>&1); then
  ...
  python3 -m json.tool "$INFONAME/.nift/public/q\"name.info.json" >/dev/null 2>&1 || fail 'quoted tracked name corrupts generated page info JSON'
fi
...
if (cd "$INFOTPL" && "$NIFT_BIN" track qtpl 'Quoted template' 'templates/q"template.html' >/dev/null 2>&1); then
  ...
  python3 -m json.tool "$INFOTPL/.nift/public/qtpl.info.json" >/dev/null 2>&1 || fail 'quoted template path corrupts generated page info JSON'
fi
...
if (cd "$INFOINPUT" && "$NIFT_BIN" build-all >/dev/null 2>&1); then
  python3 -m json.tool "$INFOINPUT/.nift/public/index.info.json" >/dev/null 2>&1 || fail 'quoted @input path corrupts generated page info JSON'
fi
...
if (cd "$WEXTQUOTE" && "$NIFT_BIN" watch content/w/ '."x' templates/template.html .html >/dev/null 2>&1); then
  python3 -m json.tool "$WEXTQUOTE/.nift/.watch/content/w/exts.json" >/dev/null 2>&1 || fail 'quoted watched content extension corrupts exts.json'
fi
...
if (cd "$WOUTQUOTE" && "$NIFT_BIN" watch content/w/ .html templates/template.html '."out' >/dev/null 2>&1); then
  python3 -m json.tool "$WOUTQUOTE/.nift/.watch/content/w/exts.json" >/dev/null 2>&1 || fail 'quoted watched output extension corrupts exts.json'
fi
...
if (cd "$DEPQUOTE" && "$NIFT_BIN" build-all >/dev/null 2>&1); then
  python3 -m json.tool "$DEPQUOTE/.nift/public/index.info.json" >/dev/null 2>&1 || fail 'quoted @dep path corrupts generated page info JSON'
fi
...
python3 - "$TMP_ROOT/info-multi.log" <<'PY' || fail 'info repeats requested names instead of reporting each once'
import json, sys
...
if (cd "$QNAME" && "$NIFT_BIN" track 'quo"te' 'Quote name' >/dev/null 2>&1); then
  python3 -m json.tool "$QNAME/.nift/tracked.json" >/dev/null 2>&1 || fail 'track name containing double quote corrupts tracked.json'
...
if (cd "$QWATCH" && "$NIFT_BIN" watch 'content/q"dir/' >/dev/null 2>&1); then
  python3 -m json.tool "$QWATCH/.nift/.watch/watched.json" >/dev/null 2>&1 || fail 'watch directory containing double quote corrupts watched.json'
...
if (cd "$CPQUOTE" && "$NIFT_BIN" cp / 'copy"quote' >/dev/null 2>&1); then
  python3 -m json.tool "$CPQUOTE/.nift/tracked.json" >/dev/null 2>&1 || fail 'cp destination containing double quote corrupts tracked.json'
fi
...
if (cd "$MVQUOTE" && "$NIFT_BIN" mv / 'move"quote' >/dev/null 2>&1); then
  python3 -m json.tool "$MVQUOTE/.nift/tracked.json" >/dev/null 2>&1 || fail 'mv destination containing double quote corrupts tracked.json'
fi
...
python3 -m json.tool "$ESCJSON/.nift/tracked.json" >/dev/null 2>&1 || fail 'track title containing backslashes corrupts tracked.json'
...
python3 - "$TMP_ROOT/slash-title.log" <<'PY' || fail 'tracked title backslashes do not round-trip through JSON'
import json, sys
with open(sys.argv[1], encoding='utf-8') as f:
    doc = json.load(f)
...
if (cd "$INITQUOTE" && "$NIFT_BIN" init '.\"bad' >/dev/null 2>&1); then
  python3 -m json.tool "$INITQUOTE/.nift/config.json" >/dev/null 2>&1 || fail 'init accepts extension containing quote then writes invalid config.json'
fi
