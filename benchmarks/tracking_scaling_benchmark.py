...
Builds synthetic tracked.json files and measures `nift info --tracking`, which opens
...
def measure(n):
    with tempfile.TemporaryDirectory(prefix=f"nift-track-{n}-") as td:
        ...
        for _ in range(args.runs):
            t=time.perf_counter()
            p=subprocess.run([args.nift,"info","--tracking"],cwd=root,
...
def expect_invalid(entries,label):
    with tempfile.TemporaryDirectory(prefix='nift-track-invalid-') as td:
        ...
        p=subprocess.run([args.nift,'info','--tracking'],cwd=root,stdout=subprocess.DEVNULL,stderr=subprocess.PIPE)
